...
export default function AgentsView() {
  ...
  const filteredAgents = agents.filter((ag) => {
    const matchesSearch =
      ag.hostname.toLowerCase().includes(search.toLowerCase()) ||
      ...
      <div className="bg-white dark:bg-[#1E293B] p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500">
            Surveillez l'état des agents Wazuh, Elastic, Winlogbeat installés
            sur vos serveurs et postes clients.
          </p>
        </div>
            ...
            <div className="space-y-3 font-mono text-xs">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-amber-500">
                  {agents.filter((a) => a.status === "UNSTABLE").length}
                </span>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse font-mono text-xs">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30 text-[10px] font-bold uppercase text-slate-400 dark:text-slate-500">
                  ...
                  <th className="py-3 px-5">Type Source</th>
                  ...
                  filteredAgents.map((ag) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 px-5 text-center">
                          <span
                            className={`px-2 py-0.5 rounded text-[9px] font-bold border uppercase ${statusConfig.style}`}
                          >
                            {statusConfig.label}
                          </span>
                        </td>
